module Riddler
  class Video
    ...
    def self.find_by_tag(session, tag, options={})
      response = session.client.get("viddler.videos.getByTag", options.merge(:tag => tag))
      Riddler::VideoList.new(session, response, 'viddler.videos.getByTag', :tag => tag)
    end
